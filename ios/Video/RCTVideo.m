            ...
            if (certificateStringUrl != nil) {
                ...
                dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0), ^{
                    ...
                    if (certificateData != nil) {
                        ...
                        NSData *contentIdData = [[NSData alloc] initWithBase64EncodedString:contentId  options:NSDataBase64DecodingIgnoreUnknownCharacters];
                        AVAssetResourceLoadingDataRequest *dataRequest = [loadingRequest dataRequest];
                        if (dataRequest != nil) {
                            NSError *spcError = nil;
                            NSDictionary *options = nil;
                            if(licenseShouldPersist != nil && [licenseShouldPersist boolValue]){
                                options =  @{AVAssetResourceLoadingRequestStreamingContentKeyRequestRequiresPersistentKey : [NSNumber numberWithBool:YES]};
                            }
...
- (void)performAuthTokenRequestWithContentId:(NSString *) contentId completion:(void (^ _Nonnull)(NSString *, NSError *))completion {
    ...
    if(drmAuthTokenURL == nil || azamToken == nil){
        ...
        NSError *error = [NSError errorWithDomain: @"RCTVideo"
                                         ...
                                         userInfo: @{
                                             ...
                                             NSLocalizedRecoverySuggestionErrorKey: errMsg
                                         }
                          ];
    ...
    NSString *urlString = [NSString stringWithFormat:@"%@?contentId=%@",drmAuthTokenURL,contentId];
    NSURL *url = [NSURL URLWithString:urlString];
    NSMutableURLRequest *request = [NSMutableURLRequest requestWithURL:url];
    [request setHTTPMethod:@"GET"];
    
    NSString *bearerToken = [NSString stringWithFormat:@"Bearer %@",azamToken];
    ...
    NSURLSessionDataTask *requestTask = [session dataTaskWithRequest:request completionHandler:^(NSData * _Nullable data, NSURLResponse * _Nullable response, NSError * _Nullable error) {
        ...
        id successObj =[jsonDict objectForKey:@"success"];
        if(successObj != nil && [successObj boolValue] == true){
...
- (void)performContentKeyRequestWithContentURL:(NSURL *) url spc:(NSData *) spcData authToken:(NSString*) authToken completion:(void (^ _Nonnull)(NSData *, NSError *))completion {
    
    NSString *licenseServer = (NSString *)[self->_drm objectForKey:@"licenseServer"];
    
    if(licenseServer == nil){
        NSString *errMsg = @"Missing licenseServer";
        NSError *error = [NSError errorWithDomain: @"RCTVideo"
                                             code: RCTVideoErrorFromJSPart
                                         userInfo: @{
                                             NSLocalizedDescriptionKey: errMsg,
    ...
    NSURLSessionDataTask *postDataTask = [session dataTaskWithRequest:request completionHandler:^(NSData *data, NSURLResponse *response, NSError *error) {
        ...
        if (error != nil) {
            ...
            if([httpResponse statusCode] != 200){
                ...
                NSString *ckcMessage =[jsonDict objectForKey:@"CkcMessage"];
                if (ckcMessage != nil) {
                    //                                      NSString *ckcMessage = (NSString *) ckcObj;
                    NSData *decodedData = [[NSData alloc] initWithBase64EncodedString:ckcMessage options:NSDataBase64DecodingIgnoreUnknownCharacters];
